...
namespace ThesisMain
{
    class Communication
    {
        ...
        public bool BringWindowToTop(string windowName, bool wait)
        {
            int hWnd = FindWindow(windowName, wait);
            if (hWnd != 0)
            {
                return Win32API.SetForegroundWindow((IntPtr)hWnd);
            }
            return false;
        }
        ...
        public static int FindWindow(string windowName, bool wait)
        {
            ...
            while (wait && (int)hWnd == 0)
            {
                System.Threading.Thread.Sleep(500);
                hWnd = Win32API.FindWindow(null, windowName);
            }
